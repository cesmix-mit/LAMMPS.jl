...
function lammps_close(handle)
    ccall((:lammps_close, liblammps), Cvoid, (LAMMPSPtr,), handle)
end
...
function lammps_file(handle, file)
    ccall((:lammps_file, liblammps), Cvoid, (LAMMPSPtr, Ptr{Cchar}), handle, file)
end

function lammps_command(handle, cmd)
    ccall((:lammps_command, liblammps), Ptr{Cchar}, (LAMMPSPtr, Ptr{Cchar}), handle, cmd)
end

function lammps_commands_list(handle, ncmd, cmds)
    ccall((:lammps_commands_list, liblammps), Cvoid, (LAMMPSPtr, Cint, Ptr{Ptr{Cchar}}), handle, ncmd, cmds)
end

function lammps_commands_string(handle, str)
    ccall((:lammps_commands_string, liblammps), Cvoid, (LAMMPSPtr, Ptr{Cchar}), handle, str)
end

function lammps_get_natoms(handle)
    ccall((:lammps_get_natoms, liblammps), Cdouble, (LAMMPSPtr,), handle)
end

function lammps_get_thermo(handle, keyword)
    ccall((:lammps_get_thermo, liblammps), Cdouble, (LAMMPSPtr, Ptr{Cchar}), handle, keyword)
end

function lammps_extract_box(handle, boxlo, boxhi, xy, yz, xz, pflags, boxflag)
    ccall((:lammps_extract_box, liblammps), Cvoid, (LAMMPSPtr, Ptr{Cdouble}, Ptr{Cdouble}, Ptr{Cdouble}, Ptr{Cdouble}, Ptr{Cdouble}, Ptr{Cint}, Ptr{Cint}), handle, boxlo, boxhi, xy, yz, xz, pflags, boxflag)
end

function lammps_reset_box(handle, boxlo, boxhi, xy, yz, xz)
    ccall((:lammps_reset_box, liblammps), Cvoid, (LAMMPSPtr, Ptr{Cdouble}, Ptr{Cdouble}, Cdouble, Cdouble, Cdouble), handle, boxlo, boxhi, xy, yz, xz)
end

function lammps_memory_usage(handle, meminfo)
    ccall((:lammps_memory_usage, liblammps), Cvoid, (LAMMPSPtr, Ptr{Cdouble}), handle, meminfo)
end

function lammps_get_mpi_comm(handle)
    ccall((:lammps_get_mpi_comm, liblammps), Cint, (LAMMPSPtr,), handle)
end

function lammps_extract_setting(handle, keyword)
    ccall((:lammps_extract_setting, liblammps), Cint, (LAMMPSPtr, Ptr{Cchar}), handle, keyword)
end

function lammps_extract_global_datatype(handle, name)
    ccall((:lammps_extract_global_datatype, liblammps), Cint, (LAMMPSPtr, Ptr{Cchar}), handle, name)
end

function lammps_extract_global(handle, name)
    ccall((:lammps_extract_global, liblammps), Ptr{Cvoid}, (LAMMPSPtr, Ptr{Cchar}), handle, name)
end

function lammps_extract_atom_datatype(handle, name)
    ccall((:lammps_extract_atom_datatype, liblammps), Cint, (LAMMPSPtr, Ptr{Cchar}), handle, name)
end

function lammps_extract_atom(handle, name)
    ccall((:lammps_extract_atom, liblammps), Ptr{Cvoid}, (LAMMPSPtr, Ptr{Cchar}), handle, name)
end

function lammps_extract_compute(handle, id, arg3, arg4)
    ccall((:lammps_extract_compute, liblammps), Ptr{Cvoid}, (LAMMPSPtr, Ptr{Cchar}, Cint, Cint), handle, id, arg3, arg4)
end

function lammps_extract_fix(handle, arg2, arg3, arg4, arg5, arg6)
    ccall((:lammps_extract_fix, liblammps), Ptr{Cvoid}, (LAMMPSPtr, Ptr{Cchar}, Cint, Cint, Cint, Cint), handle, arg2, arg3, arg4, arg5, arg6)
end

function lammps_extract_variable(handle, arg2, arg3)
    ccall((:lammps_extract_variable, liblammps), Ptr{Cvoid}, (LAMMPSPtr, Ptr{Cchar}, Ptr{Cchar}), handle, arg2, arg3)
end
...
function lammps_gather_atoms(handle, name, type, count, data)
    ccall((:lammps_gather_atoms, liblammps), Cvoid, (LAMMPSPtr, Ptr{Cchar}, Cint, Cint, Ptr{Cvoid}), handle, name, type, count, data)
end

function lammps_gather_atoms_concat(handle, name, type, count, data)
    ccall((:lammps_gather_atoms_concat, liblammps), Cvoid, (LAMMPSPtr, Ptr{Cchar}, Cint, Cint, Ptr{Cvoid}), handle, name, type, count, data)
end

function lammps_gather_atoms_subset(handle, name, type, count, ndata, ids, data)
    ccall((:lammps_gather_atoms_subset, liblammps), Cvoid, (LAMMPSPtr, Ptr{Cchar}, Cint, Cint, Cint, Ptr{Cint}, Ptr{Cvoid}), handle, name, type, count, ndata, ids, data)
end

function lammps_scatter_atoms(handle, name, type, count, data)
    ccall((:lammps_scatter_atoms, liblammps), Cvoid, (LAMMPSPtr, Ptr{Cchar}, Cint, Cint, Ptr{Cvoid}), handle, name, type, count, data)
end

function lammps_scatter_atoms_subset(handle, name, type, count, ndata, ids, data)
    ccall((:lammps_scatter_atoms_subset, liblammps), Cvoid, (LAMMPSPtr, Ptr{Cchar}, Cint, Cint, Cint, Ptr{Cint}, Ptr{Cvoid}), handle, name, type, count, ndata, ids, data)
end

function lammps_gather(handle, name, type, count, data)
    ccall((:lammps_gather, liblammps), Cvoid, (LAMMPSPtr, Ptr{Cchar}, Cint, Cint, Ptr{Cvoid}), handle, name, type, count, data)
end

function lammps_gather_concat(handle, name, type, count, data)
    ccall((:lammps_gather_concat, liblammps), Cvoid, (LAMMPSPtr, Ptr{Cchar}, Cint, Cint, Ptr{Cvoid}), handle, name, type, count, data)
end

function lammps_gather_subset(handle, name, type, count, ndata, ids, data)
    ccall((:lammps_gather_subset, liblammps), Cvoid, (LAMMPSPtr, Ptr{Cchar}, Cint, Cint, Cint, Ptr{Cint}, Ptr{Cvoid}), handle, name, type, count, ndata, ids, data)
end

function lammps_scatter(handle, name, type, count, data)
    ccall((:lammps_scatter, liblammps), Cvoid, (LAMMPSPtr, Ptr{Cchar}, Cint, Cint, Ptr{Cvoid}), handle, name, type, count, data)
end

function lammps_scatter_subset(handle, name, type, count, ndata, ids, data)
    ccall((:lammps_scatter_subset, liblammps), Cvoid, (LAMMPSPtr, Ptr{Cchar}, Cint, Cint, Cint, Ptr{Cint}, Ptr{Cvoid}), handle, name, type, count, ndata, ids, data)
end

function lammps_create_atoms(handle, n, id, type, x, v, image, bexpand)
    ccall((:lammps_create_atoms, liblammps), Cint, (LAMMPSPtr, Cint, Ptr{Cint}, Ptr{Cint}, Ptr{Cdouble}, Ptr{Cdouble}, Ptr{Cint}, Cint), handle, n, id, type, x, v, image, bexpand)
end

function lammps_find_pair_neighlist(handle, style, exact, nsub, request)
    ccall((:lammps_find_pair_neighlist, liblammps), Cint, (LAMMPSPtr, Ptr{Cchar}, Cint, Cint, Cint), handle, style, exact, nsub, request)
end

function lammps_find_fix_neighlist(handle, id, request)
    ccall((:lammps_find_fix_neighlist, liblammps), Cint, (LAMMPSPtr, Ptr{Cchar}, Cint), handle, id, request)
end

function lammps_find_compute_neighlist(handle, id, request)
    ccall((:lammps_find_compute_neighlist, liblammps), Cint, (LAMMPSPtr, Ptr{Cchar}, Cint), handle, id, request)
end

function lammps_neighlist_num_elements(handle, idx)
    ccall((:lammps_neighlist_num_elements, liblammps), Cint, (LAMMPSPtr, Cint), handle, idx)
end

function lammps_neighlist_element_neighbors(handle, idx, element, iatom, numneigh, neighbors)
    ccall((:lammps_neighlist_element_neighbors, liblammps), Cvoid, (LAMMPSPtr, Cint, Cint, Ptr{Cint}, Ptr{Cint}, Ptr{Ptr{Cint}}), handle, idx, element, iatom, numneigh, neighbors)
end

function lammps_version(handle)
    ccall((:lammps_version, liblammps), Cint, (LAMMPSPtr,), handle)
end
...
function lammps_is_running(handle)
    ccall((:lammps_is_running, liblammps), Cint, (LAMMPSPtr,), handle)
end

function lammps_force_timeout(handle)
    ccall((:lammps_force_timeout, liblammps), Cvoid, (LAMMPSPtr,), handle)
end

function lammps_has_error(handle)
    ccall((:lammps_has_error, liblammps), Cint, (LAMMPSPtr,), handle)
end

function lammps_get_last_error_message(handle, buffer, buf_size)
    ccall((:lammps_get_last_error_message, liblammps), Cint, (LAMMPSPtr, Ptr{Cchar}, Cint), handle, buffer, buf_size)
end
